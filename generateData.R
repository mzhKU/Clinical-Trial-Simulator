generateData <- function(
    ...
    treatSubj=subjects,           # Number of subjects to which to allocate
    ...
    genParVCov=0,                 # * Covariance matrix for generating fixed
                                  # parameters
    ...
    treatOrder = FALSE,	          # Logical flag: should allocations be
                                  # assigned in order
    ...
    extCovFile ,	              # File from which to import (including
                                  # full or relative path)
    extCovSubset ,	              # Subset to apply to data
    extCovRefCol ,	              # Reference variable
    extCovSameRow = TRUE,	      # Logical flag: should covariates sampled
    ...
    genParCrit,	                  # Range of acceptable values for
                                  # generated parameters
    genParBtwNames ,	          # Between subject effects to generate
    genParBtwMean ,	              # Means for generated between subject effects
    genParBtwVCov ,	              # Covariance matrix for generated
                                  # between subject effects
    genParErrStruc = "None",      # Function to map generated effects:
                                  # Additive, Proportional or None
    genParMaxDraws = 100,         # Maximum number of iterations to
                                  # generate valid parameters
    genParRangeTolerance = .5,    # Proportion of subjects with "in range"
                                  # parameters that we'd be happy proceeding with
    extParFile,                   # File name for external parameter data to import
    extParNames,	              # Names of parameters to import from external file
    extParBtwNames,	              # Between subject effects variables to
                                  # import from external file
    extParBtwNums,                # Integer mapping between random and fixed
                                  # effects in imported parameter data
    extParSubset = NULL,          # Subsets to be applied to imported parameter
                                  # before sampling
    extParCrit,	                  # Range of acceptable values for generated
                                  # parameters
    extParErrStruc = "None",	  # Function to map effects from imported
                                  # parameter data: Additive, Proportional or None
    extParRefColData,	          # Reference column in imported parameter data
    extParRefColName,	          # Reference column name from imported
                                  # parameter data
    ...
    respVCov,	                  # * Residual error (co)variance to apply
                                  # to generated response
    respErrStruc = "Additive",	  # Function describing how to apply residual
    ...
    interimSubj ,	              # * Proportion of total subjects to be assigned
                                  # to each interim analysis
    interimMethod = "Sample",	  # Method for creating interim variable:
                                  # Sample or Proportion
    ...
    parBtwSuffix = ".Between",    # Suffix for retained between subject
                                  # effects variables
    ...
    defNames <- c("treatSubj",      "treatType",    "respName", 
                  "treatOrder",     "conCovCrit",   "conCovDigits", "conCovMaxDraws",
                  "extCovSameRow",  "extCovDataId", "timeCovCrit", 
                  "genParErrStruc", "respDist",     "respErrStruc", "respDigits",
                  "genParVCov",     "mcarProp",     "dropFunExtraArgs",
                  "interimMethod",  "seed",         "idCol",        "doseCol",
                  "timeCol",        "trtCol",       "parOmitFlag",  "respOmitFlag",
                  "missingFlag",    "interimCol",   "parBtwSuffix", "deleteCurrData",
                  "covDiff",        "treatDiff",    "extParDataId")
    ...
    covList      <- innerCallList(c(subjects="subjects",
                                    conNames="conCovNames",
                                    conMean="conCovMean",
                                    conCov="conCovVCov",
                                    conRange="conCovCrit",
                                    conDigits="conCovDigits",
                                    conMaxDraws="conCovMaxDraws",
                                    disNames="disCovNames",
                                    disValues="disCovVals",
                                    disProbs="disCovProb",
                                    disProbArray="disCovProbArray",
                                    extNames="extCovNames",
                                    extFile="extCovFile",    
                                    extSubset="extCovSubset",
                                    extRefCol="extCovRefCol",
                                    extSameRow="extCovSameRow",
                                    extDataId="extCovDataId",
                                    idCol="idCol",
                                    workingPath="workingPath",
                                    timeNames="timeCovNames",
                                    timeMean="timeCovMean",
                                    timeCov="timeCovVCov",
                                    timeRange="timeCovCrit",
                                    timeCol="timeCol",
                                    timePeriod="treatPeriod"))

	parList      <- innerCallList(c(subjects="subjects",
                                    genNames="genParNames",
                                    genFixedMean="genParMean",
                                    genFixedCov="genParVCov",
                                    genRange="genParCrit",
                                    genBetweenNames="genParBtwNames",
                                    genBetweenMean="genParBtwMean",
                                    genBetweenCov="genParBtwVCov", 
                                    genErrStruc="genParErrStruc",
                                    genMaxDraws="genParMaxDraws",
                                    genParRangeTolerance="genParRangeTolerance",
                                    extFile="extParFile",
                                    extNames="extParNames",
                                    extBetween="extParBtwNames",
                                    extBetweenNums="extParBtwNums",
                                    extSubset="extParSubset",
                                    extRange="extParCrit",
                                    extErrStruc="extParErrStruc",
                                    extRefCol="extParRefColData",
                                    extRefColName="extParRefColName",
                                    extDataId="extParDataId",
                                    suffix="parBtwSuffix",
                                    idCol="idCol",
                                    flagName="parOmitFlag",
                                    workingPath="workingPath"))

     respList <- innerCallList(c(   equation="respEqn",
                                    name="respName",
                                    invLink="respInvLink",
                                    distribution="respDist",
                                    covariance="respVCov",
                                    errStruc="respErrStruc",
                                    range="respCrit",
                                    digits="respDigits",
                                    flagName="respOmitFlag"))

     mcarList <- innerCallList(c(   prop="mcarProp",
                                    rule="mcarRule",
                                    flagName="missingFlag" )) 

     dropList <- innerCallList(c(   dropFunc="dropFun",
                                    idCol="idCol",
                                    timeCol="timeCol",
                                    flagName="missingFlag"))

  interimList <- innerCallList(c(   subjects="subjects",
                                    proportion="interimSubj",
                                    idCol="idCol",
    ...
    if(!covDiff) {
        covData <- do.call(createCovariates, covList)
    }
    ...
    # }
     ...
     for (i in replicateN) { 
         ...
         if (treatDiff) {
             allocData <- do.call(allocateTreatments, allocateList)
             allocData <- allocData[rep(1:nrow(allocData),
                                    length = subjects), , drop = FALSE]
             allocData[[idCol]] <- 1:subjects
         }
         ...
         if(!missing(extParRefColData) && length(extParRefColData) == 1 &&
            is.character(extParRefColData) && !length(grep(",", extParRefColData))) {
         ...
         if(is.data.frame(respData) && nrow(respData) == nrow(coreData)) {
             coreData <- cbind(coreData, respData)
         } else {
