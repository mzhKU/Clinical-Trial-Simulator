...
# ----------------------------------------------------
emax.fit<- function(y, dose, iparm = c("ed50", "e0", "emax"),
                   ed50cutoff = 2.5 * max(doselev)) {
    # Dose mean and dose SD.
    ...
    return(list(fit = fit, fitpred = fitpred, 
           sdpred = sdpred, sddif = sddif))
}
...
SeEmax3<-function (fit, doselev) 
{
    ...
    sddif <- sqrt((fitdif^2) * (vfcov[3, 3]/coef(fit)[3]^2 + 
        vfcov[1, 1]/(coef(fit)[1] + doselev)^2 - (2 * vfcov[1, 3])/(coef(fit)[3] * (coef(fit)[1] + doselev))))
    covint <- -((doselev * coef(fit)[3])/(coef(fit)[1] + doselev)^2) * 
        vfcov[2, 1] + (doselev/(coef(fit)[1] + doselev)) * vfcov[2, 3]
