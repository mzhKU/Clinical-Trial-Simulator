"analyzeRep" <- function(
    analysisCode,                            # File containing the actual analysis code
                                             # to run on the data
    replicate,                               # Replicate number of data to analyze
    removeMissing=TRUE,                      # Logical flag: remove rows where
                                             # the "Missing"
                                             # Flag is set to 1?
    removeParOmit=TRUE,                      # Logical flag: remove rows where the
                                             # "Parameter Omit" Flag is set to 1?
    removeRespOmit=TRUE,                     # Logical flag: remove rows where the
                                             # "Reponse Omit" Flag is set to 1?
    interimCode=NULL,                        # Interim analysis Code to run on the
                                             # data between interims (eg. can be used to
                                             # drop doses)
    software=c("R", "SAS"),                  # Software system in which the analysis
                                             # should take place: R or SAS
    ...
    initialDoses=NULL,                       # Initial set of doses to use in
                                             # "interim 1"
    stayDropped=TRUE,                        # Dose dropping flag: if a dose is dropped,
                                             # should it stay dropped?
    ...
    if(class(software) == "try-error") {
        ectdStop("The software should be `R` or `SAS`")
    }
    
    # Checking the macro code.
    if(software == "R") {
    	if(class(analysisCode) == "function") {
            analysisCode <- .checkFun( analysisCode, "data" )
        } else {
    		if(!file.exists(file.path(workingPath, analysisCode))) {
                ectdStop(paste("Cannot find R analysis script file \"",
                               analysisCode, "\"", sep=""))
    	    }
        }
    } else {
    	if (!file.exists(file.path(workingPath, analysisCode))) {
            ectdStop(paste("Cannot find SAS analysis script file \"",
                           analysisCode, "\"", sep=""))
        }
    }
    
    # Checks on inputs.
    doseCol <- parseCharInput(doseCol, expected=1, valid=TRUE,
    ...
    if(removeParOmit) {
    	...
    	if ( class(valid) == "try-error" ) {
            ectdStop("Invalid format for parameter omit flag variable name")
        }
    	if (!(parOmitFlag %in% columns)) {
            removeParOmit <- FALSE
        }
    }
    ...
    if(removeRespOmit) {
        ...
        if(class(valid) == "try-error") {
            ectdStop("Invalid format for response omit flag variable name")
        }
        if(!(respOmitFlag %in% columns)) {
            removeRespOmit <- FALSE
        }
    }
    ...
    if(removeMissing) {	
    	...
    	if(class(valid) == "try-error") {
            ectdStop("Invalid format for 'Missing' flag variable name")
        }
    	if(!(missingFlag %in% columns)) {
            removeMissing <- FALSE
        }
    }
    
    interimCol <- parseCharInput(interimCol, expected = 1,
                                 convertToNumeric = FALSE, valid = TRUE)
    valid <- try( validNames( interimCol ), silent = TRUE )
    
    if ( class(valid) == "try-error" ) {
        ectdStop("Invalid format for interim allocation variable name")
    }
    if (!(interimCol %in% columns) ||
        !is.numeric(idata[[interimCol]]) ||
        any(idata[[interimCol]] < 0)) {
            interimCode <- NULL
    }
    ...
    removeSub <- NULL                
    if( removeParOmit ) {
        removeSub <- c( removeSub , paste( "( ", parOmitFlag  , " != 1 ) ", sep = "") )
    }
    if( removeMissing ) {
        removeSub <- c( removeSub , paste( "( ", missingFlag , " != 1 ) ", sep = "") ) 
    }
    if( removeRespOmit) {
        removeSub <- c( removeSub , paste( "( ", respOmitFlag , " != 1 ) ", sep = "") )
    }
    if (length(removeSub)) {
    ...
    # Perform full analysis of the data including all
    # the doses found in the data (if required).
    ...
    if (!is.null(interimCode)) {
        ...
        if (class(interimCode) == "try-error") {
            ectdStop("Cannot find the interimCode function")
        }
        ...
        for(int in 1:nInterim) {
        	...
        	newAnalysis <- try( 
        			...
        			silent = TRUE )
        	if (class(newAnalysis) == "try-error") {
                ectdStop("Error when executing `performAnalysis`\n\t$newAnalysis")
            }
            ...
            if(is.data.frame(newAnalysis) && (nRows <- nrow(newAnalysis))) {
        	    ...
        	    newAnalysis <- data.frame(
        	    	...
        	    	newAnalysis, 
        	    	stringsAsFactors = FALSE)
        	    if(doseCol %in% names(newAnalysis)) {
                  newAnalysis$INCLUDED <- as.numeric(newAnalysis[[doseCol]] %in% includeDoses)
                }
        	    ...
        	    if(dropKeep) {
        	    	if ("DROP" %in% names(iList)) {
        	    		...
        	    		if (length(iList$KEEP) != length(doses)) {
                            msg <- paste("Logical 'drop doses' return from",
                                         "interim analysis not of correct length", sep=" ")
                            ectdStop(msg)
                        }
                ...
                if (doseCol %in% names(newAnalysis)) {
                    newAnalysis$DROPPED <- as.numeric(newAnalysis[[doseCol]] %in% dropped)
                }
        	    newAnalysis$STOPPED = rep(1*("STOP" %in% names(iList) && iList$STOP ),
                                          nrow(newAnalysis))
                ...
                # Stop the trial if specified
        	    if ( "STOP" %in% names(iList) && iList$STOP ) break	

                # Stop the trial if no doses to include in next interim
        	    if (!length(includeDoses)) break
            }
        }
    }
