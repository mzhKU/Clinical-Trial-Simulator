"writeData" <- function(dat,            # Data to write, should be a data frame
                        dataNumber,     # The number of the data entry
                        dataType = c("ReplicateData",
                                     "MacroEvaluation", "MicroEvaluation"),
                                        # Type of data ("Replicate", "Micro" or "Macro"
                        # dataType = c("ReplicateData", "Micro", "Macro"),
                                        # Type of data ("Replicate", "Micro" or "Macro"
                        workingPath = getwd(), # Working directory 
                        append = FALSE, # If TRUE, this routine will append
...
)
{
  ...
  # Author: Francisco
  ###############################################################################
  # DESCRIPTION: Writes a single entry of data to a file.
  ...
  if(!is.data.frame(dat)) {
      ectdStop("dat is not a data frame")
  }
  ...
  switch(method, 
		  "CSV" = {
              ...
              fullPath <- .dataGetFullPath(dataNumber = dataNumber,
                                           dataType = dataType,
                                           workingPath = workingPath,
                                           method = method,
                                           prefix = prefix)
              # Write the file
              tryWrite <- try(write.table(dat, fullPath, append = append,
