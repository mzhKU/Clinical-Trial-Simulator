...
".checkFun" <- function(fun, expectedArgs) {
    ...
    if(!missing(expectedArgs)) {  
		expectedArgs <- parseCharInput(expectedArgs,
...
".checkReplicates" <- function(replicates, workingPath = getwd(),
    ...
    # If 'try' fails.
	if(class(repsExist) == "try-error") {
        ectdStop("No replicates found")
    }

    if(is.character(replicates)) {
        if(length(replicates) == 1 && replicates == "*") {
            return(repsExist)
        } else {
            ectdStop("'replicates' input in the wrong format")
	    }
    }
	if(is.numeric(replicates)) {
        if(!all(replicates %in% repsExist)) {
            ectdStop("Could not find all replicates specified")
        } else {
            return(replicates)
        }
    } else {
        msg <- paste("'replicates' must be a numeric vector",
                     " or a single character ('*')", sep=" ")
        ectdStop(msg)
    }
	NULL
}
