"analyzeData" <- function(
    ...
    waitAndCombine = TRUE,                     # Wait for all analyses to finish,
                                               # then combine into single file?
    cleanUp = FALSE,                           # Delete micro/macro directories
                                               # on completion?
    ...
    sleepTime = 15,                            # Number of seconds to sleep
                                               # between checking for grid jobs
    deleteCurrData = TRUE,                     # Delete current analysis results
                                               # before executing
    initialDoses = NULL,					   # Initial doses to use for "Interim 1"
    stayDropped = TRUE,				           # Dose dropping flag: if a dose
                                               # is dropped, should it stay dropped?
...
)
{
    ...
    replicates <- .checkReplicates(replicates,
                                   workingPath = workingPath,
                                   method = method)
    
    if (grid && !.checkGridAvailable()) {
        grid <- FALSE
    }
    if (length(replicates) == 1) {
        grid <- waitAndCombine <- FALSE
    }
    
    # Check directories.
    if (deleteCurrData) {
        removeDirectories(c("MicroEvaluation", "MacroEvaluation"),
                          workingPath = workingPath)
    ...
    if(grid){
        ...
        if(is.numeric(getOption("max.clusters"))) {
            nclusters <- min(nclusters, getOption("max.clusters"))
        }
        cl <- parallel:::makeCluster(nclusters)
        stopCluster <- parallel:::stopCluster
        repSplit    <- .splitGridVector(
                                       replicates,
                                       ceiling(length(replicates) / nclusters )
                       )
        ...
        clusterApply(cl=cl, repSplit, function(l,a){
            for (i in l) {
                microData <- analyzeRep(replicate = i,
                                        analysisCode = a$analysisCode, 
    					                interimCode = a$interimCode,
                                        software = a$software,
                                        removeMissing = a$removeMissing, 
    					                removeParOmit = a$removeParOmit,
                                        removeRespOmit = a$removeRespOmit, 
    				                    seed = a$seed + i,
                                        parOmitFlag = a$parOmitFlag,
                                        respOmitFlag = a$respOmitFlag, 
                                        missingFlag = a$missingFlag,
                                        interimCol = a$interimCol,
                                        doseCol = a$doseCol, 
                                        initialDoses = a$initialDoses,
                                        stayDropped = a$stayDropped,
                ...
                if (is.data.frame(microData) && nrow(microData)) {
                	...
                	macroData <- macroEvaluation(microData,
                                                 macroCode = a$macroCode, 
                                                 interimCol = a$interimCol,
                                                 doseCol = a$doseCol)
                	writeData(macroData, i, "Macro", workingPath = a$workingPath)
                } else {
                    ectdWarning(paste("No return output from replicate", i))
                }
    		}
        }, arguments)
        stopCluster(cl)
        # Store time at grid evaluation.
        evalTime <- Sys.time()
    } else {
