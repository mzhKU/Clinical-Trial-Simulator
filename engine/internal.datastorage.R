.dataGetFullPath <- function (
    ...
    dataType = c("ReplicateData", "MicroEvaluation", "MacroEvaluation"),
                           # Type of data, should be "Replicate", "Macro", or "Micro"
    #dataType = c("ReplicateData", "EvalMicro", "EvalMacro"),
                           # Type of data, should be "Replicate", "Macro", or "Micro"
    workingPath = getwd(), # The working directory
    dirName = dataType,    # Directory to use in case one doesn't want to
...
)
{
  ...
  # Author: Francisco
  # DESCRIPTION: Returns a string containing the full path of the file which has
  #              a specified entry (1-9999) 
  # of specified data type (Replicate, Micro or Macro)
  # KEYWORDS:misc, IO 
  # -----------------------------------------------------------------------------

  dataType <- match.arg(dataType)
  if (any(dataNumber < 1 || dataNumber > 9999)) {
      ectdStop("dataNumber out of range, must be between 1 and 9999")
  }
